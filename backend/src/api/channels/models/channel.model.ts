...
export class Channel {
	...
	constructor(id: number, isPrivate: boolean, isDm: boolean, title?: string, owner?: userId, password?: string) {
		this.id = id;
		this.title = title;
		this.isDm = isDm;
	...
	assertCanUserEditChannel(user: ChannelUser, password: string) {
		if (this.isDm) {
			...
			!user ||
			this.getChannelUserRole(user) == 'normal'
		  ) {
			throw {
			  code: HttpStatus.FORBIDDEN,
...
export class ChannelModel {
	...
	async mute(user: ChannelUser, channel: Channel, mutedUser: ChannelUser, seconds: number) {
		const now = new Date();
